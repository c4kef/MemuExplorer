...
public class WClient
{
    ...
    public async Task Init(bool waitQr, string pathToWeb)
    {
        ...
        {
            var status = 0;
            while (!File.Exists(@$"{Globals.TempDirectory.FullName}\{TaskId}.png") && status++ < 60)
                await Task.Delay(500);
        ...
        bool TryDeleteQR()
        {
            try
            {
                if (File.Exists(@$"{Globals.TempDirectory.FullName}\{TaskId}.png"))
                    File.Delete(@$"{Globals.TempDirectory.FullName}\{TaskId}.png");
    ...
    public async Task<bool> JoinGroup(string group)
    {
        ...
        try
        {
            ...
            _socket.Emit("data",
                JsonConvert.SerializeObject(new ServerData()
                { Type = "joinGroup", Values = new List<object>() { $"{NameSession}@{id}", group } }));
