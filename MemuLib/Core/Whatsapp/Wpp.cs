...
namespace WPP4DotNet
{
    public abstract class IWpp
    {
        ...
        /// </summary>
        /// <returns>Return True or False</returns>
        public Task<bool> IsMainReady()
        {
            try
            {
                return Task.FromResult(js.ExecuteReturnBool(Driver, "return await WPP.conn.isMainReady()"));
            }
            catch (Exception)
            {
                return Task.FromResult(false);
        ...
        public async Task<Models.SendReturnModels> SendMessage(string chat, string message, bool simulateTyping=false)
        {
            try
            {
                ...
                dynamic response = js.ExecuteReturnObj(Driver, $"return await WPP.chat.sendTextMessage('{chat}', '{message}', {{\n  createChat: true\n}});");
